...
const OtherProfile = (props) => {
    ...
    async function getUserDetails(data) {
        try {
            const response = await fetch('http://localhost:5000/userProfile/get-details', {
                method: "POST",
                headers: {
    ...
    return (
        <div>
            <Header displayName={name} picture={picture} setAuth={props.setAuth} logout={logout} currentPage='asdf' />
            <div className='profile'>
                <div>
                    ...
                    {request === "accepted" &&
                        <h4>You are friends with {userName}</h4>
                    }
                    {request === null &&
                ...
                <div className={classes.root}>
                    {userPicture ? <Avatar src={userPicture} className={classes.large}/> : <Avatar className={classes.large}/>}
                </div>              
